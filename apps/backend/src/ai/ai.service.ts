...
export interface ChatResponse {
  ...
  errorChain?: { provider: string; error: string }[];
  cacheHit?: boolean;
}
export interface ChatOptions {
  strategy?: 'cost' | 'quality' | 'balanced' | 'specific';
  provider?: string;
  temperature?: number;
  maxTokens?: number;
}
...
export class AiService {
  ...
  private bootstrapProviders() {
    const disableGemini =
      (this.config.get<string>('DISABLE_GEMINI') || '').toLowerCase() === 'true';
    const list: ChatProvider[] = [
      ...
      new XAIProvider(
        this.config.get<string>('XAI_API_KEY') || this.config.get<string>('GROK_API_KEY'),
    ...
    this.logger.log(
      'AI providers: ' +
        this.providers.map(p => `${p.meta.name}:${p.isEnabled() ? 'on' : 'off'}`).join(', ')
    );
  }
  ...
  status() {
    return {
      providers: this.providers.map(p => ({
        ...
        enabled: p.isEnabled(),
        cost: p.meta.cost,
        qualityTier: p.meta.qualityTier,
      })),
    };
  }
  ...
  private buildCacheKey(messages: ChatMessage[], opts: ChatOptions) {
    ...
    return JSON.stringify({
      s: system?.content?.slice(0, 120),
      u: lastUser?.content?.slice(0, 400),
      strat: opts.strategy,
      prov: opts.provider,
  ...
  async chat(messages: ChatMessage[], options: ChatOptions = {}): Promise<ChatResponse> {
    ...
    if (!candidates.length) {
      return {
        reply:
          'No AI providers active. Add an API key (e.g. OPENAI_API_KEY or XAI_API_KEY) and try again.',
        model: 'offline',
        provider: 'none',
      };
    }
    ...
    if (cached && cached.expires > Date.now()) return { ...cached.response, cacheHit: true };
    ...
    for (const p of candidates) {
      try {
        ...
        const out: ChatResponse = {
          ...resp,
          fallbackTried: errors.map(e => e.provider),
    ...
    return {
      reply: 'All AI providers failed. Try again later.',
      model: 'unavailable',
      provider: 'none',
      errorChain: errors,
    };

...
export class AnthropicProvider implements ChatProvider {
  ...
  async chat(messages: ChatMessage[], opts: { temperature?: number; maxTokens?: number }): Promise<ChatResponse> {
    ...
    try {
      ...
      return { reply: text, model: this.meta.model, usage: { inputTokens: resp.data.usage?.input_tokens, outputTokens: resp.data.usage?.output_tokens }, provider: this.meta.name };
    } catch (e:any) {

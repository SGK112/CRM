  ...
  async chat(messages: ChatMessage[], opts: { temperature?: number; maxTokens?: number }): Promise<ChatResponse> {
    ...
    try {
      const resp = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [ system ? { role: 'system', content: system.content } : undefined, ...rest.map(m=> ({ role: m.role, content: m.content })) ].filter(Boolean),
        temperature: opts.temperature ?? 0.6,
        max_tokens: opts.maxTokens ?? 512
      }, { headers: { Authorization: `Bearer ${this.apiKey}` } });
      const choice = resp.data.choices?.[0]?.message?.content || 'No reply produced.';
      return { reply: choice, model: this.meta.model, usage: { inputTokens: resp.data.usage?.prompt_tokens, outputTokens: resp.data.usage?.completion_tokens }, provider: this.meta.name };
    } catch (e:any) {

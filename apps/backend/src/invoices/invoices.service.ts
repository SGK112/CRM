...
export interface CreateInvoiceDto { clientId: string; projectId?: string; items: { name:string; description?:string; quantity?:number; unitPrice?:number; taxable?:boolean; }[]; taxRate?: number; notes?: string; dueDate?: string; }
export interface UpdateInvoiceDto { items?: { name:string; description?:string; quantity?:number; unitPrice?:number; taxable?:boolean; }[]; taxRate?: number; notes?: string; dueDate?: string; }
export interface RecordPaymentDto { amount: number; note?: string; date?: string; }
...
export class InvoicesService {
  ...
  private async generateInvoicePDF(invoice: InvoiceDocument, client?: { firstName?: string; lastName?: string; email?: string; company?: string }): Promise<Buffer> {
    ...
    doc.moveDown();
    doc.text('Items:');
    (invoice.items||[]).forEach((item: { name: string; quantity: number; unitPrice?: number; total?: number }, idx: number) => {
      const unit = (item.unitPrice ?? 0).toFixed(2);
      const lineTotal = (item.total ?? (item.quantity||1) * (item.unitPrice||0)).toFixed(2);
      doc.text(`${idx + 1}. ${item.name} - Qty: ${item.quantity} - Unit: $${unit} - Total: $${lineTotal}`);
    });
    doc.moveDown();
  ...
  async send(id: string, workspaceId: string) {
    ...
    await this.emailService.sendEmail({
      ...
      attachments: [
        { filename: `Invoice-${doc.number}.pdf`, content: pdfBuffer },
      ],
    });
  ...
  async getPdf(id: string, workspaceId: string): Promise<{ buffer: Buffer; filename: string } | null> {
    ...
    const clientInfo = client ? { firstName: client.firstName, lastName: client.lastName, email: client.email, company: client.company } : {};

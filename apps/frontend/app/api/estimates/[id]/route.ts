...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/estimates/${id}`, {
      method: 'PATCH', // Backend uses PATCH for updates
      headers,
    ...
    if (!response.ok) {
      ...
      const errorData = await response.json().catch(() => ({ message: 'Failed to update estimate' }));
      return NextResponse.json(
        { error: errorData.message || 'Failed to update estimate' },
        { status: response.status }
      );
    }

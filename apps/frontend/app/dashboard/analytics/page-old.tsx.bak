...
export default function AnalyticsPage() {
  ...
  <div key={metric.title} className="surface-1 rounded-xl shadow-sm border border-token p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-gray-700 >{metric.period}</p>
        </div>
      </div>
    </div>
  );
            ...
            return (
              <div key={data.month} className="flex-1 flex flex-col items-center">
                ...
                <div className="mt-2 text-center">
                  ...
                  <p className="text-xs text-gray-700 dark:text-[var(--text-dim)]">${(data.revenue / 1000).toFixed(0)}k</p>
                </div>
              </div>
            );
          })}
        ...
        {projectsByType.map((project, index) => (
          <div key={project.type} className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-800 w-12 text-right">{project.count}</span>
              <span className="text-sm text-gray-700 w-8 text-right">{project.percentage}%</span>
            </div>
          </div>
        ))}
      </div>
        ...
        {clientSources.map((source, index) => (
          <div key={source.source} className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-800 w-8 text-right">{source.count}</span>
              <span className="text-sm text-gray-700 w-12 text-right">{source.percentage}%</span>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-brand-700 dark:text-brand-400">Analytics Dashboard</h1>

...
// API message payload shape from backend
type ApiMessage = Omit<InboxMessage, never>;
...
export default function InboxPage() {
  ...
  const response = await fetch(`/api/inbox?${queryParams}`, {
      ...
      if (response.ok) {
        ...
        // Use the messages directly from the unified inbox API
        const messages: InboxMessage[] = Array.isArray(data.messages)
          ? (data.messages as ApiMessage[]).map((message: ApiMessage) => ({
              _id: message._id,
              type: message.type,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          localStorage.removeItem('inboxComposeDraft');
          setDraftCount(0);
        } catch {
          // ignore
        }
  ...
  const response = await fetch('/api/inbox/stats', {
      ...
      if (response.ok) {
        ...
        setStats({
          ...
          byType: {
            email: data.byType?.email || 0,
            notification: data.byType?.notification || 0,
            sms: data.byType?.sms || 0,
            system: data.byType?.system || 0,
          },
          byPriority: {
            urgent: data.byPriority?.urgent || 0,
            high: data.byPriority?.high || 0,
            normal: data.byPriority?.normal || 0,
  ...
  // Load draft count on mount and when compose data changes
  useEffect(() => {
    try {
      const stored = localStorage.getItem('inboxComposeDraft');
      if (stored) {
        setDraftCount(1);
      } else {
        setDraftCount(0);
  ...
  const toggleStar = async (messageId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessages(prev =>
          prev.map(msg => (msg._id === messageId ? { ...msg, isStarred: !msg.isStarred } : msg))
        );
        fetchStats();
      }
    } catch (error) {
    ...
    <div className={simple.page()}>
      ...
      {noAuth && (
        <div className="mb-4 p-3 rounded-lg bg-yellow-50 text-yellow-800 border border-yellow-200">
          You are not logged in. Please sign in to view your inbox.
        </div>
      )}
      {/* Mobile Header */}
      ...
      <div className="hidden lg:block mb-8">
        <h1 className={simple.text.title()}>Inbox</h1>
        <p className={simple.text.body()}>
          Manage your messages, notifications, and communications
        </p>
      </div>

      {/* Main Content */}
      <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] lg:h-[calc(100vh-12rem)]">
        {/* Mobile: Message List (Full Width) */}
        <div className="flex-1 lg:w-96 lg:border-r lg:border-gray-200 lg:dark:border-gray-700">
          {/* Search Bar */}
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 lg:bg-white lg:dark:bg-gray-800">
          ...
          <div className="lg:hidden p-4 border-b border-gray-200 dark:border-gray-700">
            <div className="flex gap-2 overflow-x-auto">
              ...
              <button
                onClick={() => setFilters({ ...filters, type: 'email' })}
                className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${
                  filters.type === 'email'
                    ? 'bg-blue-600 text-white'
                    : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
              ...
              <button
                onClick={() => setFilters({ ...filters, type: 'notification' })}
                className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <button
              ...
              className="flex items-center justify-between w-full text-sm font-medium text-gray-700 dark:text-gray-300 mb-4 py-2 px-3 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
            >
              <span>Filters</span>
              <ChevronDown
              ...
              {loading ? (
                ...
                messages.map(message => (
                  <div
                    ...
                    className={`border-b border-gray-200 dark:border-gray-700 p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-3">
                            ...
                            {message.metadata?.attachments &&
                              message.metadata.attachments.length > 0 && (
                                <span className="flex items-center space-x-1">
                                  ...
                                  <span>{message.metadata.attachments.length}</span>
                                </span>
                              )}
                          </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            >
                              <Star
                                className={`w-4 h-4 ${message.isStarred ? 'fill-current' : ''}`}
                              />
                            </button>
                            <button
                              onClick={e => {
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        <div className="flex-shrink-0">{getTypeIcon(message.type)}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            {!message.isRead && (
                            ...
                            {message.priority !== 'normal' && (
                              <span
                                className={`px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${getPriorityColor(message.priority)}`}
                              >
                                {message.priority}
                              </span>
                            )}
              ...
              <div className="h-full flex flex-col">
                ...
                <div className="px-8 py-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-750">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-6 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        {selectedMessage.priority !== 'normal' && (
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(selectedMessage.priority)}`}
                          >
                    ...
                    <div className="flex items-center space-x-2 ml-6">
                      ...
                      >
                        <Star
                          className={`w-5 h-5 ${selectedMessage.isStarred ? 'fill-current' : ''}`}
                        />
                      </button>
                      <button
                      ...
                      <button
                        ...
                        disabled={actionLoading === selectedMessage._id}
                        className="p-3 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-red-500 hover:text-red-600"
                      >
                ...
                <div className="flex-1 overflow-y-auto px-8 py-6">
                  ...
                  {selectedMessage.metadata?.attachments &&
                    selectedMessage.metadata.attachments.length > 0 && (
      ...
      {showFilters && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          ></div>
          <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 rounded-t-xl p-6">
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="space-y-4">
                  ...
                  {typeof window !== 'undefined' && localStorage.getItem('inboxComposeDraft') && (
                    <div className="p-3 rounded-md bg-blue-50 text-blue-800 border border-blue-200 flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e =>
                        setComposeData({ ...composeData, type: e.target.value as 'email' | 'sms' })
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        composeData.type === 'email' ? 'recipient@example.com' : '+1234567890'
                      }
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    />
                  </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button
                  ...
                  onClick={() => {
                    try {
                      localStorage.setItem('inboxComposeDraft', JSON.stringify(composeData));
                      setDraftCount(1);
                      setShowCompose(false);
                    } catch {
                      // ignore

...
export default function NewInvoice() {
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      ...
      if (clientsRes.ok) {
        const clientsResponseData = await clientsRes.json();
        // Handle API response format {success: true, data: [...]}
        const clientsData = clientsResponseData.data || clientsResponseData.clients || clientsResponseData || [];
        setClients(
          clientsData.map((c: { name?: string; firstName: string; lastName: string; [key: string]: unknown }) => ({
            ...c,
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {selectedClientId && (
                <>
                  ...
                  <button
                    type="button"
                    onClick={() => router.push(`/dashboard/clients/${selectedClientId}?source=invoice_new`)}
              ...
              <button
                type="button"
                onClick={() => router.push('/dashboard/estimates/new' + (selectedClientId ? `?clientId=${selectedClientId}` : '') + (selectedProjectId ? `&projectId=${selectedProjectId}` : ''))}

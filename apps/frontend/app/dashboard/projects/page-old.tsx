...
export default function ProjectsPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'status' | 'priority' | 'budget' | 'updated'>(
    'updated'
  );
        ...
        <PageHeader
          ...
          actions={
            <Link
              ...
              className="pill pill-tint-blue sm inline-flex items-center gap-2 transition-transform hover:scale-105"
            >
          ...
          {[
            {
              key: 'all',
              label: 'All Projects',
              color: 'pill-tint-neutral',
              count: projects.length,
            },
            {
              key: 'planning',
              label: 'Planning',
              color: 'pill-tint-blue',
              count: statusCounts.planning || 0,
            },
            {
              key: 'active',
              label: 'Active',
              color: 'pill-tint-green',
              count: statusCounts.active || 0,
            },
            {
              key: 'on_hold',
              label: 'On Hold',
              color: 'pill-tint-yellow',
              count: statusCounts.on_hold || 0,
            },
            {
              key: 'completed',
              label: 'Completed',
              color: 'pill-tint-gray',
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-red-300 bg-red-50 p-8 text-sm text-red-700 space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => fetchProjects()} className="pill pill-tint-red sm">
                Retry
              </button>
              <button
                onClick={() => {
                ...
                className="pill pill-tint-neutral sm"
              >
                Dismiss
              </button>
            </div>
          ...
          <div className="text-center py-20 bg-white border border-dashed border-gray-300 rounded-xl">
            ...
            <p className="text-gray-800 dark:text-gray-300 mb-8 max-w-md mx-auto">
              Adjust your filters or create a new project to get started with tracking your
              construction work.
            </p>
            <Link
              href="/dashboard/projects/new"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map(project => (
              <div
                key={project._id}
                className="group bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg hover:border-blue-200 transition-all duration-300 flex flex-col overflow-hidden"
              >
                <div className="p-6 flex-1 flex flex-col">
                  {/* Header */}
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">
                        {project.title}
                      </h3>
                      <p className="text-gray-800 dark:text-gray-300 text-sm line-clamp-2">
                        {project.description}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    <span
                      className={`pill sm pill-tint-${project.status === 'active' ? 'green' : project.status === 'planning' ? 'blue' : project.status === 'on_hold' ? 'yellow' : project.status === 'cancelled' ? 'red' : 'neutral'}`}
                    >
                      {project.status.replace('_', ' ')}
                    </span>
                    <span
                      className={`pill sm pill-tint-${project.priority === 'high' ? 'yellow' : project.priority === 'urgent' ? 'red' : project.priority === 'medium' ? 'blue' : 'neutral'}`}
                    >
                      {project.priority}
                    </span>
                  </div>
                  ...
                  <div className="space-y-3 text-sm text-gray-800 dark:text-gray-200 flex-1">
                    ...
                    {project.assignedUsers && project.assignedUsers.length > 0 && (
                      <div className="flex items-center bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
                        ...
                        <span>
                          {project.assignedUsers.length} team member

...
export default function VendorsPage() {
  ...
  useEffect(() => {
    fetchVendors(); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <div className="space-y-8 p-6">
        ...
        actions={
          <div className="flex gap-2">
            <button
              ...
              className="pill pill-tint-blue inline-flex items-center gap-1 text-xs"
            >
            ...
            <button
              ...
              className="pill pill-tint-green inline-flex items-center gap-1 text-xs"
            >
              <PlusIcon className="h-4 w-4" />
              {showNew ? 'Close' : 'New Vendor'}
            </button>
          </div>
        }
      />
      ...
      <div className="surface-1 border border-token rounded-xl p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="md:col-span-3 flex flex-col">
            <label className="text-[11px] font-medium uppercase tracking-wide block mb-1 text-slate-500">
              Search
            </label>
          ...
          <div className="flex flex-col">
            ...
            <select
              ...
              className="input pr-8 w-full min-w-[160px]"
            >
          ...
          {/* Actions */}
          <div className="flex flex-col">
            <label className="text-[11px] font-medium uppercase tracking-wide block mb-1 text-slate-500 opacity-0 select-none">
              Reset
            </label>
            <div className="flex gap-2">
              <button
                ...
                className="pill pill-tint-gray text-xs inline-flex items-center gap-1 flex-1 justify-center"
              >
        ...
        <div className="surface-1 border border-dashed border-token rounded-xl p-4 space-y-3 animate-in fade-in">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="pill pill-tint-green inline-flex items-center gap-1 text-xs disabled:opacity-50"
            >
              <PlusIcon className="h-4 w-4" />
              Create
            </button>
            <button
              ...
              className="pill pill-tint-gray text-xs"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="surface-1 border border-token rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-token flex items-center justify-between">
          <h2 className="text-sm font-semibold tracking-wide text-slate-700 dark:text-[var(--text)]">
            Directory
          </h2>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-token">
                {filtered.map(v => (
                  <tr
                    key={v._id}
                    className="hover:bg-slate-50/60 dark:hover:bg-[var(--surface-2)]/60"
                  >
                    <td className="px-4 py-2 font-medium text-slate-800 dark:text-[var(--text)]">
                      {v.name}
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex flex-wrap gap-1">
                        {(v.categories || ['—']).map(c => (
                          <span
                            key={c}
                            className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-[var(--surface-2)] text-[10px] font-medium text-slate-700 dark:text-[var(--text-dim)] border border-slate-200 dark:border-[var(--border)]"
                          >
                            {c}
                          </span>
                        ))}

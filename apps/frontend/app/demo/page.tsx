...
const demoSteps = [
  {
    ...
    description:
      'Capture a lead, qualify scope, and spin up a standardized project shell in minutes.',
    outcomes: [
      'Lead captured',
      'Scope baseline stored',
      'Initial budget range',
      'Risk flags identified',
    ],
    features: [
      'Lead intake form → project',
      'Auto scope summary (AI)',
  ...
  },
  {
    ...
    description:
      'Generate a detailed take‑off, refine cost categories, and issue a polished client proposal.',
    outcomes: [
      'Itemized take‑off',
  ...
  },
  {
    ...
    description:
      'Field + office synced: schedule tasks, track costs, manage RFIs, and log daily progress.',
    outcomes: [
      'Live schedule updates',
      'Cost vs budget visibility',
      'Daily logs captured',
      'Issue / RFI tracking',
    ],
    features: [
      'Task board & Gantt hybrid',
      'Live budget burn chart',
      'Mobile daily log capture',
  ...
  },
  {
    ...
    description:
      'Punchlists resolved, documents archived, performance analytics fed back into estimating.',
    outcomes: ['Punchlist cleared', 'Docs archived', 'Variance analysis', 'Lessons learned module'],
    features: [
      'Close‑out checklist',
      'Auto document bundling',
      'Margin variance drilldown',
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-20 pb-16 text-center">
          ...
          <p className="max-w-3xl mx-auto text-lg md:text-xl text-slate-400 mb-8">
            Follow a realistic construction project lifecycle. Each step highlights outcomes, key
            screens, and how AI removes friction.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              href="/auth/register"
              className="inline-flex items-center rounded-md px-7 py-3 text-base font-medium border border-slate-600 hover:border-amber-500/60 text-slate-300 hover:text-amber-400 transition-colors"
            >
              Launch Workspace <ArrowRightIcon className="ml-2 h-4 w-4" />
            </Link>
          </div>
          <p className="mt-4 text-xs uppercase tracking-wide text-slate-500">
            4-stage scenario • Real outcomes • No fluff
          </p>
        </div>
      </header>
          ...
          <div className="flex gap-4 overflow-x-auto py-4">
            {demoSteps.map((step, index) => {
              ...
              return (
                ...
                >
                  <span
                    className={`w-6 h-6 flex items-center justify-center rounded-md text-[11px] font-semibold ${active ? 'bg-white/15' : 'bg-slate-700/80 text-slate-300'}`}
                  >
                    {index + 1}
                  </span>
                ...
                <ul className="space-y-2">
                  {demoSteps[activeStep].features.map((f, i) => (
                    <li key={i} className="flex items-start text-sm text-slate-300">
                      <span className="mt-1 mr-2 inline-flex h-5 w-5 items-center justify-center rounded bg-amber-600/20 ring-1 ring-amber-600/40 text-amber-400 text-[11px] font-semibold">
                        {i + 1}
                      </span>
            ...
            <div className="flex flex-wrap gap-4">
              {activeStep > 0 && (
                <button
                  ...
                  className="px-5 py-2 text-sm rounded-md border border-slate-700 hover:border-amber-600/60 text-slate-300 hover:text-amber-400 transition"
                >
                  Previous
                </button>
              )}
              {activeStep < demoSteps.length - 1 ? (
                <button
                  onClick={() => setActiveStep(activeStep + 1)}
                  className="px-5 py-2 text-sm rounded-md bg-amber-600 hover:bg-amber-500 text-white font-semibold inline-flex items-center gap-2 shadow shadow-amber-600/30 transition"
                >
              ...
              <div className="relative h-96 p-5 grid grid-cols-12 gap-4 text-xs">
                ...
                <div className="col-span-4 row-span-2 rounded-md bg-slate-800/70 ring-1 ring-slate-700 flex flex-col p-3">
                  ...
                  <p className="text-slate-400 leading-snug">
                    Auto AI synopsis & key metrics snapshot.
                  </p>
                </div>
                <div className="col-span-4 row-span-3 rounded-md bg-slate-800/40 ring-1 ring-slate-700 flex flex-col p-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between">
                      <span>Schedule</span>
                      <span className="text-amber-400">+4d</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="col-span-8 row-span-2 rounded-md bg-slate-800/60 ring-1 ring-slate-700 flex flex-col p-3">
                  ...
                  <div className="flex gap-2 overflow-hidden">
                    {['Planned', 'Active', 'Review', 'Done'].map(stage => (
                      <span
                        key={stage}
                        className="px-2 py-1 rounded bg-slate-700/60 text-slate-300 text-[10px] tracking-wide"
                      >
                        {stage}
                      </span>
                    ))}
                ...
                <div className="col-span-8 row-span-2 rounded-md bg-slate-800/40 ring-1 ring-slate-700 flex flex-col p-3">
                  ...
                  <div className="flex-1 flex items-end gap-1">
                    {[50, 70, 40, 90, 65].map((h, i) => (
                      <div
                        key={i}
                        className="flex-1 bg-slate-700/60 relative overflow-hidden rounded"
                      >
                        <div
                          style={{ height: `${h}%` }}
                          className="absolute bottom-0 inset-x-0 bg-amber-500/70"
                        />
                      </div>
                    ))}
                  </div>
                ...
                {/* Tooltip overlays */}
                {showTooltips &&
                  demoSteps[activeStep].tooltips.map((t, i) => (
                    <div
                      key={i}
                      className="absolute z-10 text-[11px] pointer-events-none"
                      style={{ left: `${t.x}%`, top: `${t.y}%`, transform: 'translate(-50%,-50%)' }}
                    >
                      <div className="bg-amber-600/90 text-slate-900 font-medium px-2 py-1 rounded shadow-lg ring-1 ring-amber-400/60 whitespace-nowrap">
                        {t.text}
                      </div>
                    </div>
                  ))}
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={card.title}
              className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 flex flex-col gap-4"
            >
              <h4 className="font-semibold text-slate-100">{card.title}</h4>
              <ul className="space-y-2 text-sm text-slate-300">
                {card.points.map(p => (
                  <li key={p} className="flex items-start">
                    <CheckIcon className="h-4 w-4 text-amber-500 mr-2 mt-[2px]" /> <span>{p}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-12 flex flex-col items-center text-center gap-6 shadow-xl shadow-black/40">
            ...
            <p className="text-slate-400 max-w-2xl text-sm md:text-base">
              Deploy in minutes. Standardize process, gain real-time visibility, and feed every
              project back into smarter future estimates.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href="/auth/register"
                className="rounded-md bg-amber-600 hover:bg-amber-500 px-6 py-2.5 text-sm font-semibold shadow shadow-amber-600/30 transition"
              >
                Start Free Trial
              </Link>
              <Link
                href="/auth/login"
                className="rounded-md border border-slate-600 hover:border-amber-500/60 px-6 py-2.5 text-sm font-medium text-slate-300 hover:text-amber-400 transition"
              >
                Sign In
              </Link>
            </div>

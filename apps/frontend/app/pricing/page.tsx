...
export const metadata: Metadata = {
  ...
  description:
    'Simple, scalable pricing for construction teams. Start free and upgrade when you grow.',
};
...
const featureMatrix: {
  section: string;
  items: { label: string; tiers: (boolean | string)[] }[];
}[] = [
  {
  ...
  return (
    <MarketingPage>
      <MarketingHero
        ...
        subtitle="Start free. Upgrade only when your operation demands more."
      />
      <section className="px-4 sm:px-6 lg:px-8 pb-12">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-center gap-4 mb-10 text-xs text-slate-400">
            ...
            <div className="flex items-center gap-1">
              <CurrencyDollarIcon className="w-4 h-4 text-amber-400" /> <span>Usage-based AI</span>
            </div>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {plans.map(p => (
              <div
                key={p.name}
                className={`relative rounded-xl border ${p.highlight ? 'border-amber-500/50 ring-2 ring-amber-500/40 shadow-lg shadow-amber-600/20' : 'border-slate-800'} bg-slate-900/40 p-6 flex flex-col`}
              >
                {p.highlight && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-amber-500 text-[10px] font-semibold tracking-wide text-slate-900 shadow">
                    MOST POPULAR
                  </span>
                )}
                ...
                <div className="mb-5 flex items-end gap-1">
                  ...
                  <span className="ml-2 text-xs text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded">
                    ${p.priceYearly}/mo annual
                  </span>
                </div>
                ...
                <button
                  className={`mt-auto w-full rounded-md py-2.5 text-sm font-medium transition ${p.highlight ? 'bg-amber-600 hover:bg-amber-500 text-white shadow shadow-amber-600/30' : 'bg-slate-800/70 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-amber-500/40'}`}
                >
                  {p.cta}
                </button>
              </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-20">
        <div className="max-w-6xl mx-auto">
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-slate-900/60 text-slate-400 text-xs uppercase tracking-wide">
                <tr>
                  ...
                  {plans.map(p => (
                    <th key={p.name} className="py-3 px-4 font-medium text-center">
                      {p.name}
                    </th>
                  ))}
                </tr>
                ...
                {featureMatrix.map(group => (
                  <>
                    <tr key={group.section} className="bg-slate-800/40">
                      <td
                        colSpan={4}
                        className="py-2 px-4 text-amber-400 font-semibold text-xs tracking-wide"
                      >
                        {group.section}
                      </td>
                    </tr>
          ...
          <p className="text-[11px] text-slate-500 mt-4">
            * AI usage billed separately via token consumption. All prices in USD. Taxes may apply.
          </p>
        </div>
      </section>
    </MarketingPage>

...
const incidents = [
  {
    ...
    description:
      'We are performing routine maintenance on our mobile synchronization services. The mobile app may experience slower sync times during this period.',
    updates: [
      {
        time: '10:30 UTC',
  ...
  },
  {
    ...
    description:
      'Some users experienced slower API response times. The issue has been identified and resolved.',
    updates: [
      {
        time: '09:45 UTC',
...
export default function StatusPage() {
  const overallStatus = systems.some(s => s.status === 'outage')
    ? 'outage'
    : systems.some(s => s.status === 'degraded')
      ? 'degraded'
      : systems.some(s => s.status === 'maintenance')
        ? 'maintenance'
        : 'operational';
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium border ${getStatusColor(system.status)}`}
                      >
                        {system.status}
                      </span>
                    </div>
                    <div className="text-xs text-slate-500">Checked {system.lastChecked}</div>
                  </div>
                </div>
              </div>
            ))}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${getIncidentStatusColor(incident.status)}`}
                        >
                          {incident.status}
                        </span>
                        <span
                          className={`text-xs font-medium ${getIncidentImpactColor(incident.impact)}`}
                        >
                  ...
                  <div className="space-y-3">
                    ...
                    {incident.updates.map((update, updateIndex) => (
                      <div
                        key={updateIndex}
                        className="flex gap-3 pl-4 border-l-2 border-slate-700"
                      >
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-6 sm:p-8 text-center">
            ...
            <p className="text-slate-400 mb-6 text-sm sm:text-base">
              Subscribe to receive notifications about system status changes and scheduled
              maintenance.
            </p>

...
export default function CalendarPage() {
  ...
  const getEventColor = (status: Appointment['status'], priority: Appointment['priority']) => {
    ...
    const baseColors = {
      confirmed: { base: '#10b981', urgent: '#059669', high: '#10b981', medium: '#34d399', low: '#6ee7b7' },
      scheduled: { base: '#3b82f6', urgent: '#2563eb', high: '#3b82f6', medium: '#60a5fa', low: '#93c5fd' },
      completed: { base: '#6b7280', urgent: '#4b5563', high: '#6b7280', medium: '#9ca3af', low: '#d1d5db' },
      cancelled: { base: '#ef4444', urgent: '#dc2626', high: '#ef4444', medium: '#f87171', low: '#fca5a5' },
      rescheduled: { base: '#f59e0b', urgent: '#d97706', high: '#f59e0b', medium: '#fbbf24', low: '#fcd34d' }
    };
  ...
  const filterAppointments = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(apt =>
        ...
        (apt.location && Object.values(apt.location).some(val =>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <button
                ...
                className={`inline-flex items-center px-4 py-2.5 rounded-lg border transition-all duration-200 ${showFilters
              ...
              >
                ...
                Filters
                {(statusFilter !== 'all' || typeFilter !== 'all' || priorityFilter !== 'all' || dateRange !== 'all') && (
                  <span className="ml-2 bg-amber-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                    {[statusFilter, typeFilter, priorityFilter, dateRange].filter(f => f !== 'all').length}
                  </span>
                )}
              </button>
              ...
              <div className="flex bg-white dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600 p-1">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center px-3 py-2 text-sm font-medium rounded-md transition-all duration-200 ${view === viewOption.key
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Today's Appointments</p>
                ...
                <p className="text-xs text-gray-700 dark:text-gray-300 mt-1">
                  {filteredAppointments.filter(a =>
                    new Date(a.startDateTime).toDateString() === new Date().toDateString() && a.status === 'confirmed'
                  ).length} confirmed
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">Total Filtered</p>
                <p className="text-3xl font-bold text-black dark:text-white mt-1">{filteredAppointments.length}</p>
                <p className="text-xs text-gray-700 dark:text-gray-300 mt-1">
            ...
            >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedEvent.priority === 'urgent' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' :
                          selectedEvent.priority === 'high' ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300' :
                            selectedEvent.priority === 'medium' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
                              'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'
                        }`}>
                        {selectedEvent.priority.toUpperCase()}
                      </span>
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${selectedEvent.status === 'confirmed' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                          selectedEvent.status === 'scheduled' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' :
                            selectedEvent.status === 'completed' ? 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300' :
                              selectedEvent.status === 'cancelled' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' :
                                'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300'
                        }`}>
                        {selectedEvent.status.toUpperCase()}
                      </span>
                    </div>
                    <h3 className="text-xl font-bold text-black dark:text-white">{selectedEvent.title}</h3>
                  </div>
                  <button
              ...
              <div className="px-6 py-6 space-y-6">
                {selectedEvent.description && (
                  <div>
                    <h4 className="text-sm font-semibold text-black dark:text-white mb-2">Description</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      ...
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-white">Appointment Type</div>
                  ...
                  <div className="space-y-4">
                    {selectedEvent.location && (
                      <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-800 dark:text-gray-200">
                            {selectedEvent.location.street}<br />
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      ...
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-white">Reminders</div>
                        <div className="text-sm text-gray-800 dark:text-gray-200">

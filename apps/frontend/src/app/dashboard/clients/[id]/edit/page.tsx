...
export default function EditClientPage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/clients/${id}`, { method:'PATCH', headers: authHeaders(), body: JSON.stringify(body) });
      if (res.ok) {
      ...
      <form onSubmit={save} className="max-w-5xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <Link href={`/dashboard/clients/${id}`} className="px-4 py-2 text-sm rounded-md border border-gray-300 bg-white hover:bg-gray-50">Cancel</Link>
            <button disabled={saving} type="submit" className="inline-flex items-center px-5 py-2 rounded-md text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"><CheckIcon className="h-5 w-5 mr-1"/>{saving? 'Saving...':'Save'}</button>
          </div>
        </div>

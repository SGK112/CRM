...
export default function FinancialDocumentsPage() {
      ...
      const mockEstimates: Estimate[] = [
        {
          ...
          terms: 'Payment: 50% deposit, 25% at rough-in, 25% on completion. All materials guaranteed.',
          aiInsights: {
  ...
  const filterDocuments = () => {
    let filtered = documents.filter(doc => {
      ...
      const matchesProjectType = projectTypeFilter === 'all' || doc.projectType === projectTypeFilter;
  ...
  const handleQuickBooksSync = async (document: Document) => {
    if (!qbEnabled) {
      setError('QuickBooks integration is not enabled. Please configure it in Settings > Integrations.');
      return;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-800 dark:text-gray-300">Total {mode === 'estimate' ? 'Estimates' : 'Invoices'}</p>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    document.type === 'estimate' 
                      ? estimateStatusColors[document.status as keyof typeof estimateStatusColors] || 'bg-gray-100 text-gray-800'
                      : invoiceStatusColors[document.status as keyof typeof invoiceStatusColors] || 'bg-gray-100 text-gray-800'
                  }`}>
                    {document.status}
                  </span>
                </div>
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg mb-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium text-purple-800 dark:text-purple-200">
                      AI Confidence: {Math.round((document as Estimate).aiInsights!.confidenceScore * 100)}%
                    </span>
                  </div>

...
export default function RolladexPage() {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {/* Filters and Search */}
      <div className="bg-[var(--surface-1)] p-6 rounded-lg shadow border border-[var(--border)]">
        <div className="flex flex-col sm:flex-row gap-4">
          {/* Search */}
          <div className="flex-1">
            <div className="relative">
              <MagnifyingGlassIcon className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--text-faint)]" />
              <input
                type="text"
                placeholder="Search contacts, companies, titles..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="block w-full rounded-md border-0 py-2 pl-10 pr-3 bg-[var(--input-bg)] text-[var(--text)] ring-1 ring-inset ring-[var(--border)] placeholder:text-[var(--text-faint)] focus:ring-2 focus:ring-inset focus:ring-[var(--accent)] sm:text-sm sm:leading-6 transition-colors"
              />
            </div>
          </div>

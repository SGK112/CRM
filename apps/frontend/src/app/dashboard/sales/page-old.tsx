      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-800 dark:text-[var(--text-dim)]">Estimates, invoices & payment performance</p>
          </div>
          ...
          <>
            ...
            <div className="surface-1 rounded-xl border border-token p-4 mt-6">
              ...
              <div className="space-y-2">
                ...
                {filteredPayments.length === 0 && <div className="text-xs text-gray-700 dark:text-gray-300">No payments in selection.</div>}
              </div>
            </div>
          </>
        )}
              ...
              <tbody className="divide-y divide-[var(--border)]/60">
                {estimates.map(e => (
                  <tr key={e.id} className="hover:bg-[var(--surface-2)]/60">
                    ...
                    <td className="py-2 px-3 text-[11px] text-gray-700 dark:text-gray-300">{e.project || '-'}</td>
              ...
              <tbody className="divide-y divide-[var(--border)]/60">
                {invoices.map(i => (
                  <tr key={i.id} className="hover:bg-[var(--surface-2)]/60">
                    ...
                    <td className="py-2 px-3 text-[11px] text-gray-700 dark:text-gray-300">{i.project || '-'}</td>
              ...
              <tbody className="divide-y divide-[var(--border)]/60">
                {payments.map(p => (
                  <tr key={p.id} className="hover:bg-[var(--surface-2)]/60">
                    ...
                    <td className="py-2 px-3 text-[11px] text-gray-700 dark:text-gray-300">{p.source}</td>
...
function StatCard({ icon: Icon, label, value, sub, tint = 'blue' }: StatCardProps) {
  ...
  return (
    <div className="surface-1 rounded-xl border border-token p-4 flex flex-col gap-3">
      ...
      <div className="text-xs uppercase tracking-wide text-gray-700 dark:text-gray-300 font-medium">{label}</div>

...
export default function VoiceAgentPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'agents' | 'analytics' | 'settings'>('overview');
        ...
        <CapabilityGate need="voice.agents">
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => setSelectedAgent(VOICE_AGENTS.find(a => a.id === e.target.value) || VOICE_AGENTS[0])}
                      ...
                      <>
                        ...
                        {status && (
                          <div className="text-xs text-gray-600 dark:text-gray-400">
                            {status.twilio.configured ? (
                              <p>✅ Twilio connected. Calls from <span className="font-medium">{status.twilio.from}</span></p>
                            ) : (
                        ...
                        {call && (
                          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 text-sm">
                            <p><span className="font-medium">Status:</span> {call.status} {call.simulated && <span className="text-xs">(simulated)</span>}</p>
                    ...
                    {inboundOnly && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        <h3 className="font-medium text-blue-800 dark:text-blue-200 mb-2">Inbound Only Mode</h3>
              ...
              <div>
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Active Agent</h3>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{selectedAgent.voice}</p>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Calls Handled:</span>
                        <span className="font-medium text-gray-900 dark:text-white">{selectedAgent.callsHandled}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Avg Rating:</span>
                        <span className="font-medium text-gray-900 dark:text-white">{selectedAgent.avgRating}/5</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Response Time:</span>
                        <span className="font-medium text-gray-900 dark:text-white">{selectedAgent.responseTime}s</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'agents' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {VOICE_AGENTS.map(agent => (
                <div key={agent.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">{agent.description}</p>
                  ...
                  <div className="space-y-2 text-sm border-t border-gray-200 dark:border-gray-700 pt-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">{agent.specialty}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Calls:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{agent.callsHandled}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Rating:</span>
                      <div className="flex items-center gap-1">
                        <StarIcon className="w-4 h-4 text-yellow-500" />
                        <span className="font-medium text-gray-900 dark:text-white">{agent.avgRating}</span>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'analytics' && (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Voice Agent Analytics</h2>
              <div className="text-center py-12">
                <ChartBarIcon className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Advanced Analytics</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  Detailed call analytics, performance metrics, and conversation insights coming soon.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 dark:text-white">Call Volume Trends</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Track daily/weekly patterns</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 dark:text-white">Conversion Rates</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Measure call effectiveness</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 dark:text-white">Sentiment Analysis</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Customer satisfaction insights</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'settings' && (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Voice Agent Configuration</h2>
              
              <div className="space-y-6">
                <div>
                  <h3 className="text-md font-medium text-gray-900 dark:text-white mb-3">Twilio Configuration</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">TWILIO_ACCOUNT_SID</code>
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">TWILIO_AUTH_TOKEN</code>
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">TWILIO_PHONE_NUMBER</code>
                    </div>
                    <div className="text-gray-600 dark:text-gray-400">
                      <p>Configure these environment variables in your backend to enable real phone calls.</p>
                ...
                <div>
                  <h3 className="text-md font-medium text-gray-900 dark:text-white mb-3">ElevenLabs Configuration</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">ELEVENLABS_API_KEY</code>
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">ELEVENLABS_VOICE_ID</code>
                      <code className="block px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded">ELEVENLABS_AGENT_ID</code>
                    </div>
                    <div className="text-gray-600 dark:text-gray-400">
                      <p>Optional: Enhanced voice features and conversational AI capabilities.</p>
                      <p className="mt-2">Includes widget embedding and advanced voice synthesis.</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${status.twilio.configured ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
                        Twilio: {status.twilio.configured ? `Connected (${status.twilio.from})` : 'Not configured - using simulation'}
                      </p>
                      <p className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${status.elevenlabs.configured ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                        ElevenLabs: {status.elevenlabs.configured ? `Configured (agent ${status.elevenlabs.agentId})` : 'Optional - not configured'}
                      </p>
                    </div>
                  </div>
                )}

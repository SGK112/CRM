...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-6 text-center bg-[var(--bg)]">
      ...
      <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-[var(--text)] mb-3">Something went wrong</h1>
      ...
      {error?.digest && (
        <p className="mt-6 text-[10px] uppercase tracking-wide text-[var(--text-faint)]">Error ID: {error.digest}</p>
      )}
    </div>
  );
}

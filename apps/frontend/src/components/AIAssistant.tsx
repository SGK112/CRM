...
export default function AIAssistant({ isOpen, onClose, initialMessage }: AIAssistantProps) {
          ...
          const initial: AIMessage[] = [
            {
              id: 'sys-1',
              type: 'system',
              content: 'Assistant session started. Context awareness enabled. Type / for commands.',
              timestamp: new Date(),
              meta: { role },
            },
            {
              id: 'a-1',
              type: 'assistant',
              content: `${personalGreeting} I'm your Remodely Ai Assistant. I adapt answers using your workspace context. Try /recent, /projects, /clients, /new-project, /summary or /help. What's next?`,
              timestamp: new Date(),
              suggestions: [
  ...
  const processAIResponse = (aiReply: string, userQuery: string): AIMessage => {
    ...
    return {
      ...
      suggestions:
        actions.length === 0 ? ['Tell me about features', 'Show demo', 'Start trial'] : undefined,
    };
  };
  ...
  const handleAIError = (status: number) => {
    ...
    switch (status) {
      ...
      case 503:
        errorMessage =
          'AI service is temporarily unavailable. I can still help with basic navigation and tasks.';
  ...
  const handleTimeoutError = () => {
    setMessages(prev => [
      ...prev,
      {
        ...
        type: 'assistant',
        content:
          'The AI response took too long. Let me help you with quick navigation while we resolve this.',
        timestamp: new Date(),
  ...
  const handleConnectionError = () => {
    setMessages(prev => [
      ...prev,
      {
        ...
        type: 'assistant',
        content:
          'Connection issue detected. I can still help you navigate the CRM while we reconnect.',
        timestamp: new Date(),
  ...
  const handleGenericError = () => {
    setMessages(prev => [
      ...prev,
      {
        ...
        type: 'assistant',
        content:
          'Something went wrong with the AI service. I can still help you navigate and manage your CRM data.',
        timestamp: new Date(),
  ...
  const parseUserIntent = (raw: string): AIMessage => {
    ...
    if (input.startsWith('/')) {
      ...
      switch (command) {
        case 'recent': {
          ...
          try {
            pages = JSON.parse(localStorage.getItem(PAGE_HISTORY_KEY) || '[]');
          } catch (err) {
            void err;
          }
          if (pages.length === 0) {
            responseContent =
              "No recent page history yet. Navigate around the dashboard and I'll remember.";
          } else {
            const top = pages
              .slice(0, 6)
            ...
            actions = pages
              ...
              .map(p => ({
                type: 'navigate',
                label: p.path.replace('/dashboard/', '') || 'dashboard',
        ...
        case 'reset': {
          ...
          responseContent =
            "Conversation memory cleared. I'll still remember pages you visit. Type /help to start fresh.";
          break;
        }
        case 'help':
          responseContent =
          ...
          actions = [
            {
              ...
              description: 'Open quick project form',
              data: { cmd: 'new-project' },
              icon: PlusIcon,
            },
            {
              type: 'command',
              label: 'Show Summary',
              description: 'Projects & clients counts',
              data: { cmd: 'summary' },
              icon: ClipboardDocumentListIcon,
            },
          ];
        ...
        case 'projects':
          ...
          actions.push(
            ...
            },
            {
              type: 'command',
              label: 'New Project',
              description: 'Open quick create form',
              data: { cmd: 'new-project' },
              icon: PlusIcon,
            }
          );
          ...
          actions.push(
            {
              type: 'navigate',
              label: 'Open Clients',
              description: 'Go to clients page',
              data: { path: '/dashboard/clients' },
              icon: UserGroupIcon,
            },
            {
              type: 'command',
              label: 'Add Client',
              description: 'Open clients page new form',
              data: { cmd: 'open-new-client' },
              icon: PlusIcon,
            }
          );
          break;
    ...
    if (lowerInput.includes('dashboard') || lowerInput.includes('home')) {
      ...
      responseContent =
        'I can take you to the dashboard where you can see an overview of all your projects and activities.';
      ...
      if (lowerInput.includes('create') || lowerInput.includes('new')) {
        ...
        responseContent =
          'I can help you create a new project. This will open the project creation form where you can add details like client information, timeline, and budget.';
      } else {
        actions.push({
          type: 'navigate',
      ...
      lowerInput.includes('business card') ||
      lowerInput.includes('rolladex') ||
    ...
    // General help - but don't provide actions for business questions that should go to AI
      ...
      const businessQuestionPattern =
        /(how.*sell|sell.*more|increase.*sales|business.*advice|marketing|strategy|grow.*business|lead.*generation|kitchen.*remodel|bathroom.*remodel)/i;
    ...
    return {
      ...
      suggestions:
        actions.length === 0
          ? ['Show me projects', 'Create a new client', 'Open calendar', 'View settings']
          : undefined,
    };
  };
  ...
  const handleSend = async () => {
    ...
    const actionablePattern =
      /(create|add|new)\s+(project|client)|\b(projects?|clients?|calendar|documents?|settings?|ecommerce|store|rolladex|contacts?|help)\b/i;
    const businessQuestionPattern =
      /(how.*sell|sell.*more|increase.*sales|business.*advice|marketing|strategy|grow.*business|lead.*generation|kitchen.*remodel|bathroom.*remodel)/i;
        ...
        if (!searchHook.isLoading) {
          ...
          if (searchHook.results.length) {
            const resultMsg: AIMessage = {
              ...
              type: 'assistant',
              content:
                `Found ${searchHook.results.length} result(s):\n` +
                searchHook.results
                  .slice(0, 6)
                  .map(r => `• [${r.type}] ${r.title}`)
                  .join('\n'),
              timestamp: new Date(),
              actions: searchHook.results
                .slice(0, 6)
                .map(r => ({
                  type: 'navigate',
                  label: r.title,
            ...
            setMessages(prev => [
              ...prev,
              {
                id: genId(),
                type: 'assistant',
                content: 'No results found. Try refining your search terms.',
                timestamp: new Date(),
              },
            ]);
          }
    ...
    if (!token) {
      setMessages(prev => [
        ...prev,
        {
          id: Date.now().toString(),
          type: 'assistant',
          content: 'You are not authenticated. Please log in again.',
          timestamp: new Date(),
        },
      ]);
      return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages(prev => [
          ...prev,
          {
            id: genId(),
            type: 'assistant',
            content: `Project "${json.title}" created successfully.`,
            timestamp: new Date(),
            actions: [
              {
                type: 'navigate',
                label: 'Open Project',
                description: 'View the project detail page',
                data: { path: `/dashboard/projects/${json._id}` },
                icon: EyeIcon,
              },
            ],
          },
        ...
        setMessages(prev => [
          ...prev,
          {
            id: genId(),
            type: 'assistant',
            content: 'Failed to create project. Check required fields.',
            timestamp: new Date(),
          },
        ]);
      }
      ...
      setMessages(prev => [
        ...prev,
        {
          id: genId(),
          type: 'assistant',
          content: 'Network error creating project.',
          timestamp: new Date(),
        },
      ]);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-stretch sm:justify-end pointer-events-none">
      ...
      <div className="relative w-full sm:w-[520px] h-[80vh] sm:h-full sm:max-h-screen surface-1 shadow-2xl sm:rounded-none rounded-t-2xl flex flex-col pointer-events-auto border-l border-t sm:border-t-0 border-token overflow-hidden bg-[var(--surface-1)] text-[var(--text)]">
        {/* Header */}
        <div
          className={`flex items-center justify-between px-4 py-3 border-b border-token bg-[var(--surface-1)]/90 backdrop-blur-md supports-[backdrop-filter]:bg-[var(--surface-1)]/70 sticky top-0 z-10 transition-shadow ${scrolled ? 'shadow-sm' : ''}`}
        >
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-0.5">
                <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-200 text-amber-800 dark:bg-amber-600/25 dark:text-amber-300 border border-amber-300 dark:border-amber-500/30">
                  / for commands
                </span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-200 text-blue-800 dark:bg-blue-600/25 dark:text-blue-300 border border-blue-300 dark:border-blue-500/30">
                  {contextSummary.projects ?? '–'} projects
                </span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-green-200 text-green-800 dark:bg-green-600/25 dark:text-green-300 border border-green-300 dark:border-green-500/30">
                  {contextSummary.clients ?? '–'} clients
                </span>
              </div>
            </div>
          </div>
          <div className="flex items-center space-x-1">
            <button
              ...
              className="px-2 py-1 text-[11px] rounded-md border border-gray-400 dark:border-[var(--border)] text-black dark:text-white bg-gray-200 dark:bg-[var(--surface-2)] hover:bg-gray-300 dark:hover:bg-[var(--surface-2)] transition-colors shadow-sm"
              title="Toggle command palette"
            >
              /{showCommands ? 'hide' : 'cmds'}
            </button>
            <button
              onClick={() => {
                localStorage.removeItem(MEMORY_KEY);
            ...
            <span className="hidden sm:inline-flex px-2 py-1 text-[10px] rounded-md bg-[var(--surface-2)] text-[var(--text-dim)] border border-[var(--border)]">
              ⌘K
            </span>
            <button
              onClick={onClose}
        ...
        {/* Messages */}
        <div
          className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-[var(--surface-1)]"
        ...
        >
          ...
          {messages.map(message => (
            ...
            >
              ...
              >
                <p
                  className={`whitespace-pre-wrap leading-relaxed ${message.type === 'system' ? 'font-mono' : ''}`}
                >
                  {message.content}
                </p>
                <div className="flex items-center justify-end space-x-2 mt-2 opacity-60 group-hover:opacity-90 transition-opacity">
                  <p
                    className={`text-[10px] ${message.type === 'user' ? 'text-amber-100' : 'text-[var(--text-dim)]'}`}
                  >
                  ...
                  <div className="mt-3 flex flex-col gap-2">
                    {message.actions.map((action, index) => {
                      ...
                      return (
                        ...
                        >
                          <IconComp
                            className={`h-4 w-4 ${message.type === 'user' ? 'text-white' : 'text-[var(--text-dim)] group-hover/action:text-[var(--text)]'}`}
                          />
        ...
        <div className="border-t border-token bg-[var(--surface-2)] px-4 pt-2 pb-3 space-y-2 sticky bottom-0 z-10 shadow-[0_-2px_10px_-4px_rgba(0,0,0,0.4)]">
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex items-center gap-3 text-[10px] text-[var(--text-dim)]">
              ...
              <span>
                {Math.ceil(input.trim().split(/\s+/).filter(Boolean).length * 1.3)} tokens est.
              </span>
              {isLoading && (
                <span className="flex items-center gap-1">
                  <span className="h-3 w-3 border-2 border-amber-500 border-t-transparent rounded-full animate-spin" />{' '}
              ...
              <div className="flex justify-between mt-1 px-1">
                <div className="flex gap-2">
                  <span
                    ...
                    title="Use Ctrl/⌘ + K to toggle the assistant panel"
                  >
                    Ctrl/⌘ + K toggle panel
                  </span>
                  <span
                    className="text-[10px] text-[var(--text-dim)]"
                    title="Type / at the beginning of your message to see available commands"
                  >
                    / for commands
                  </span>
                </div>
                <span
                  className="text-[10px] text-[var(--text-dim)]"
                  title="Keyboard shortcuts for sending messages"
                >
          ...
          <div
            className="text-[10px] text-center text-[var(--text-dim)] pt-0.5"
            title="AI Assistant capabilities and features"
          >
            Remodely Ai Assistant can navigate, create entities, and summarize your workspace.
          </div>
        </div>
      </div>
    </div>

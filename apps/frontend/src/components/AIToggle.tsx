    ...
    <div className={`flex items-center gap-2 ${className}`}>
      <button
        ...
        className={`
          ...
          ${isAIEnabled ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}
      ...
      {showLabels && (
        <div className="flex items-center gap-1.5">
          <Sparkles
            className={`h-3 w-3 ${isAIEnabled ? 'text-blue-500' : 'text-gray-400'} transition-colors duration-200`}
          />
...
export function AIToggleCompact({
  ...
  onToggle,
  className = '',
}: Omit<AIToggleProps, 'size' | 'showLabels'>) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <button
        onClick={() => onToggle(!isAIEnabled)}

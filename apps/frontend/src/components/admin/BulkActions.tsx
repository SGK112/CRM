...
export default function BulkActions({
  ...
  const [selectedAction, setSelectedAction] = useState<
    'suspend' | 'activate' | 'delete' | 'update-plan' | ''
  >('');
  ...
  const handleBulkAction = async () => {
    ...
    const confirmed = confirm(
      `Are you sure you want to ${selectedAction} ${selectedUserIds.length} user(s)?`
    );
    if (!confirmed) return;
  ...
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
      <div className="mb-4">
        ...
        <p className="text-blue-700 text-sm">Select an action to perform on all selected users.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="flex items-end space-x-2">
          <button
            ...
            disabled={
              loading || !selectedAction || (selectedAction === 'update-plan' && !selectedPlan)
            }
      ...
      <div className="text-sm text-blue-700 space-y-1">
        {selectedAction === 'activate' && (
          <p>
            ✅ This will activate all selected user accounts, allowing them to log in and use the
            system.
          </p>
        )}
        {selectedAction === 'suspend' && (
          <p>
            ⚠️ This will suspend all selected user accounts, preventing them from accessing the
            system.
          </p>
        )}
        {selectedAction === 'update-plan' && (
        ...
        {selectedAction === 'delete' && (
          <p>
            🚨 <strong>WARNING:</strong> This will permanently delete all selected user accounts and

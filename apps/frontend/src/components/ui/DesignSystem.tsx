...
const toneMap: Record<
  string,
  { base: string; focus: string; hover: string; border?: string; text?: string }
> = {
  default: {
    base: 'bg-[var(--surface-2)] text-[var(--text)]',
    hover: 'hover:bg-[var(--surface-3)]',
    focus: 'focus:ring-[var(--accent)] focus:ring-2',
  ...
  warn: {
    ...
    focus: 'focus:ring-2 focus:ring-yellow-400',
  },
  success: {
  ...
  neutral: {
    base: 'bg-[var(--surface-2)] text-[var(--text-dim)]',
    hover: 'hover:bg-[var(--surface-3)]',
    focus: 'focus:ring-[var(--accent)] focus:ring-2',
    border: 'border border-[var(--border)]',
  },
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantClasses = {
    ...
    ghost:
      'bg-transparent hover:bg-[var(--surface-2)] text-[var(--text-dim)] focus:ring-2 focus:ring-[var(--accent)]',
    pill: 'pill',
  }[variant];
  ...
  return (
    <button
      className={clsx(
        'inline-flex items-center font-medium transition relative select-none',
        base,
        rad,
        variantClasses,
    ...
    >
      {loading && (
        <span className="absolute left-2 h-4 w-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
      )}
      {leftIcon && (
        <span className={clsx('mr-2 flex items-center', loading && 'opacity-30')}>{leftIcon}</span>
      )}
      <span className={loading ? 'opacity-30' : undefined}>{children}</span>
      {rightIcon && (
        <span className={clsx('ml-2 flex items-center', loading && 'opacity-30')}>{rightIcon}</span>
      )}
    </button>
  );
};
  ...
  return (
    <div
      className={clsx(
        ...
        'border-token',
        padMap[padding],
        interactive && 'transition hover:shadow-md hover:border-[var(--accent)]/40',
        className
      )}
      {...rest}
    >
...
export const Divider: React.FC<{ vertical?: boolean; className?: string }> = ({
  ...
  <div
    className={clsx(vertical ? 'w-px h-full' : 'h-px w-full', 'bg-[var(--border)]', className)}
  />
);
...
export const Heading: React.FC<{
  ...
  return (
    <Tag
      className={clsx('font-semibold tracking-tight text-[var(--text)]', sizes[level], className)}
    >
      {children}
    </Tag>
  );

...
export function StatBlock({
  ...
  return (
    ...
    >
      ...
      <span>{value}</span>
      {hint && (
        <em className="not-italic text-[10px] font-medium text-slate-400 dark:text-slate-500 mt-1">
          {hint}
        </em>
      )}
    </div>
...
export function StatsBar({ stats }: { stats: StatItem[] }) {
  ...
  return (
    <div
      className={mobileOptimized('flex flex-wrap gap-2 sm:gap-3 pt-3', mobile.scrollContainer())}
    >
      {stats.map(s => (
        <StatBlock key={s.label} label={s.label} value={s.value} hint={s.hint} />
      ))}
    </div>
  );
}

...
const SelectValue = ({ placeholder = 'Select...', className = '' }: SelectValueProps) => {
  ...
  return (
    <span
      className={`block truncate ${!value ? 'text-gray-500 dark:text-gray-400' : ''} ${className}`}
    >
      {value || placeholder}
    </span>
  );
...
const SelectContent = ({ children, className = '' }: SelectContentProps) => {
  ...
  return (
    <div
      className={`absolute top-full left-0 right-0 z-50 mt-1 max-h-60 overflow-auto rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 py-1 shadow-lg ${className}`}
    >
      {children}
    </div>
  );

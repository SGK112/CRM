...
export const ButtonVariants = {
  primary: 'bg-amber-600 text-white hover:bg-amber-700 border-amber-600 hover:border-amber-700 shadow-sm',
  secondary: 'surface-1 text-primary border-token hover:surface-2 hover:border-strong',
  outline: 'bg-transparent text-secondary border-token hover:surface-1 hover:text-primary hover:border-strong',
...
export function getStatusColor(status: string, type: 'project' | 'client' | 'priority' | 'general' = 'general'): string {
...
export function Card({ children, className = '', elevated = false }: CardProps) {
  return (
    <div className={`surface-1 border border-token rounded-lg ${elevated ? 'elevated' : ''} ${className}`}>
      {children}
    </div>
  );
}

...
export interface TakeoffResult {
  items: TakeoffItem[];
  summary: { cabinetCount: number; cabinetLF: number; countertopAreaSqFt: number; edgeLF: number };
}
...
export function computeTakeoff(state: DesignState): TakeoffResult {
  ...
  items.push({
    ...
    qty: parseFloat(cabinetLF.toFixed(2)),
    unit: 'LF',
  });
  ...
  if (countertops.length) {
    items.push({
      ...
      qty: parseFloat(areaSqFt.toFixed(2)),
      unit: 'sqft',
    });
    items.push({
      category: 'countertops',
      description: 'Countertop Edge',
      qty: parseFloat(edgeLF.toFixed(2)),
      unit: 'LF',
    });
  }
  return {
    ...
    summary: {
      cabinetCount,
      cabinetLF: parseFloat(cabinetLF.toFixed(2)),
      countertopAreaSqFt: parseFloat(areaSqFt.toFixed(2)),
      edgeLF: parseFloat(edgeLF.toFixed(2)),
    },
  };
}
